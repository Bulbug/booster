...
class GameBoostViewModelFactory(
    ...
    override fun <T : ViewModel> create(modelClass: Class<T>): T {
        return when (modelClass) {
            HomeViewModel::class.java -> HomeViewModel(app.deviceInfoManager, app.capabilityManager) as T
            GamesViewModel::class.java -> GamesViewModel(app.gameLibraryManager, app.gameProfileStore) as T
            BoostViewModel::class.java -> {
                requireNotNull(packageName) { "BoostViewModel requires a packageName" }
                BoostViewModel(packageName, app.deviceInfoManager, app.boostEngine, app.gameProfileStore) as T
            }
